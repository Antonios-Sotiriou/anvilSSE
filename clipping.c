...
/* Height Pipeline clipping algorythm. */
const MeshHeightStepTwo heightclipp(MeshHeightStepTwo *c, vec4f plane_p, vec4f plane_n) {

    MeshHeightStepTwo r = *c;
    size_t face_size = sizeof(Heightface);
    r.f = malloc(face_size);
    int index = 0;
    int dynamic_inc = 1;

    int clipped_count = 0;
    Heightface clipped[2];
    for (int i = 0; i < c->f_indexes; i++) {

        clipped_count = heightclipp_triangle(plane_p, plane_n, c->f[i], &clipped[0], &clipped[1]);
