    ...
    for (int i = 0; i < num_of_members; i++) {
        ...
        if (obj->BB.minZ > m.BB.minZ && obj->BB.minZ < m.BB.maxZ || 
            ...
            if (obj->BB.minX > m.BB.minX && obj->BB.minX < m.BB.maxX || 
                ...
                obj->rahm = 0;
                // m.pivot += obj->mvdir * DeltaTime;
                // Mat4x4 dr = translationMatrix(m.pivot[0], m.pivot[1], m.pivot[2]);
                // s->m[tf->quads[obj->quadIndex].mems[i]].v = setvecsarrayxm(m.v, m.v_indexes, dr);
