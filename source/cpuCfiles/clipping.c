...
/* Bounding box faces clipping. */
const Mesh clippBbox(Mesh *m, vec4f plane_p, vec4f plane_n) {

    Mesh r = *m;
    size_t face_size = sizeof(face);
    r.bbox.f = malloc(face_size);
    int index = 0;
    int dynamic_inc = 1;
    ...
    for (int i = 0; i < m->bbox.f_indexes; i++) {
        ...
        clipped_count = clipp_triangle(plane_p, plane_n, m->bbox.f[i], &clipped[0], &clipped[1]);

        if (clipped_count) {

            if (clipped_count == 1) {
                r.bbox.f = realloc(r.bbox.f, face_size * dynamic_inc);
                r.bbox.f[index] = clipped[0];
                index++;
                dynamic_inc++;
            } else if (clipped_count == 2) {
                r.bbox.f = realloc(r.bbox.f, face_size * (dynamic_inc + 1));
                r.bbox.f[index] = clipped[0];

                r.bbox.f[index + 1] = clipped[1];
                index += 2;
                dynamic_inc += 2;
            } else if (clipped_count == 3) {
                r.bbox.f = realloc(r.bbox.f, face_size * dynamic_inc);
                r.bbox.f[index] = m->bbox.f[i];
                index++;
                dynamic_inc++;
            }
        }
    }
    r.bbox.f_indexes = index;
    free(m->bbox.f);
    return r;
}
